...
class StorageTypeViewController: UIViewController {
    
    var selectionToPass = ""
    
    @IBAction func hddSegue(_ sender: UIButton) {
        selectionToPass = "hdd"
        performSegue(withIdentifier: "storageTypeSelected", sender: self)
    }
    
    @IBAction func ssdSegue(_ sender: UIButton) {
        selectionToPass = "ssd"
        performSegue(withIdentifier: "storageTypeSelected", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destination = segue.destination as? StorageDeviceTableViewController {
            destination.passedTypeSelection = selectionToPass
        }
    }
    
    
    // This section merely for device selection. Won't do much other than handle the selection of type and segue to the device table.
...
class StorageDeviceTableViewController: UITableViewController {
    
    var arrayOfComponents: [storageDevice] = []
    var passedTypeSelection: String = ""
    ...
    override func viewDidLoad() {
        ...
        self.title = passedTypeSelection.uppercased()
        
        group.enter()
        DownloadData(toDownload: "https://top-computer-parts.p.rapidapi.com/top/26/\(passedTypeSelection)")
